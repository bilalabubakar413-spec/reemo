...
function renderClientsGrid() {
    ...
    container.innerHTML = clients.map((client, i) => `
        <div class="card card-hover-border border cursor-pointer flex flex-col justify-between h-full" style="animation: fadeIn 0.3s ease-out ${i * 0.1}s both;">
            <div class="flex justify-between items-start mb-6">
                <div class="flex gap-4 items-center">
                    <div class="w-12 h-12 rounded-lg border border-white-10 bg-white-5 flex items-center justify-center shrink-0 overflow-hidden">
                        <img src="https://ui-avatars.com/api/?name=${client.name.replace(/ /g, '+')}&background=111&color=fff&size=48" alt="${client.name} Icon" class="w-full h-full object-cover opacity-80 mix-blend-screen">
                    </div>
                    <div>
                ...
                <div class="flex flex-col items-end gap-2">
                    ...
                    <button class="p-2 bg-white-5 hover:bg-white-10 rounded-lg border border-white-10 text-white-50 hover:text-white transition-colors mt-1" title="Download Contract">
            ...
            <div class="grid grid-cols-2 gap-4 pt-4 border-t border-white-5">
                <div>
                    <p class="text-[10px] text-white-30 uppercase font-bold tracking-wider mb-1">Developers</p>
                    <p class="text-xl font-bold">${client.developersCount}</p>
                </div>
                <div>
                    <p class="text-[10px] text-white-30 uppercase font-bold tracking-wider mb-1">Monthly Hours</p>
                    <p class="text-xl font-bold">${client.totalHoursMonth}h</p>
                </div>
            </div>
        </div>
    `).join('');
...
function renderCVDatabase() {
    ...
    tbody.innerHTML = cvs.map((cv, i) => {
        ...
        const statusClass = isOriginal ? 'bg-white-10 text-white-60' : 'bg-blue-500-10 text-blue-400 font-bold';
        
        // Actions layout
        const actions = isOriginal 
            ? `<div class="flex items-center justify-end gap-3">
                   <button class="p-2 bg-white-5 hover:bg-white-10 rounded-lg text-white-50 hover:text-white transition-colors border border-white-5"><i data-lucide="download" class="w-4 h-4"></i></button>
                   <button class="text-[10px] font-bold text-blue-400 hover:text-blue-300 transition-colors uppercase tracking-widest px-3 py-2 bg-blue-500-10 hover:bg-blue-500-20 rounded-md">CONVERT TO REEMO</button>
               </div>`
            : `<div class="flex items-center justify-end gap-3">
                   <button class="p-2 bg-white-5 hover:bg-white-10 rounded-lg text-white-50 hover:text-white transition-colors border border-white-5"><i data-lucide="download" class="w-4 h-4"></i></button>
               </div>`;
        ...
        return `
        <tr class="hover:bg-white-5 transition-all group" style="animation: fadeIn 0.2s ease-out ${i * 0.05}s both;">
            <td class="p-4">
                <div class="flex items-center gap-3">
                    <div class="w-8 h-8 rounded-lg flex flex-center text-xs font-bold bg-white-10 text-white-60 shrink-0">${getInitials(cv.name)}</div>
                    <span class="font-medium text-white-90">${cv.name}</span>
                </div>
            </td>
            <td class="p-4">
                <div class="flex flex-wrap gap-2">
                    ${cv.skills.map(skill => `<span class="px-2 py-0.5 rounded-md bg-white-10 text-white-60 text-[10px] font-medium border border-white-5">${skill}</span>`).join('')}
                </div>
            </td>
            <td class="p-4 text-white-40 text-sm font-mono">${cv.uploadDate}</td>
            <td class="p-4">
                <span class="px-2.5 py-1 rounded-md text-[10px] uppercase font-bold tracking-widest ${statusClass}">
                    ${cv.status}
                </span>
            </td>
